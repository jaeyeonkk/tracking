...
def check_eye_deviation(landmarks):
    ...
    if abs(left_white_ratio - 0.5) > deviation_threshold or abs(right_white_ratio - 0.5) > deviation_threshold:
...
def check_eye_deviation(landmarks):
    ...
    print(f"Left White Ratio: {left_white_ratio}, Right White Ratio: {right_white_ratio}")  # 이 부분을 추가
...
def face_info_route():
    ...
    return jsonify(face_count=face_count, no_face_for=no_face_for, face_changed=face_changed, eye_deviation_count=eye_deviation_count) 
